...
train_dirs = '/home/adt/data/data/huawei_class/train_new/train8'
val_dirs = '/home/adt/data/data/huawei_class/train_new/val8'
...
def main():
    ...
    criterion =nn.BCEWithLogitsLoss(pos_weight=torch.tensor([1.0], device='cuda'))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #print(output.size())
        # print("target_var.shape",target_var)
        t = torch.full_like(output, 0.05,dtype=float,device='cuda')
        # print("t::::", t)
        for k in range(target_var.shape[0]):
            if int(target_var[k])==0:
                t[k][0]=0.95
                t[k][1] = 0.05
            else:
                t[k][0]=0.05
...
def save_checkpoint(state, model_state, is_best, filename='checkpoint.pth'):
    ...
    if is_best:
        ...
        shutil.copyfile(model_file_name, 'runs/%s/'%(args.name) + 'model_best_model.pth')
...
#     lr = args.lr * (0.1 ** (epoch // 80)) * (0.1 ** (epoch // 200))
#     # log to TensorBoard
#     if args.tensorboard:
