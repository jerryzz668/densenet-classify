...
def main(Fs_epochs,Fs_lr,Fs_batchsize,Fs_layer,Fs_growth,train_path_idx,val_path_idx,Fs_accuracy,Fs_modelname):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        train_dataset,
        batch_size=Fs_batchsize, shuffle=True, **kwargs)
    ...
    # create model
    model = dn.DenseNet3(Fs_layer, 3, Fs_growth, reduction=args.reduce,
    ...
    optimizer = torch.optim.SGD(model.parameters(), Fs_lr,
    ...
    for epoch in range(args.start_epoch, Fs_epochs):
...
def save_checkpoint(state, model,is_best, filename='checkpoint.pth',filename2='CP0.pth'):
...
def adjust_learning_rate(optimizer, epoch):
    ...
    lr = Fs_lr * (0.1 ** (epoch // 150)) * (0.1 ** (epoch // 225)) * (0.1 ** (epoch // 350))
    # log to TensorBoard
    if args.tensorboard:
...
def accuracy(output, target, topk=(1,)):
    ...
    for k in topk:
        ...
        res.append(correct_k.mul_(100.0 / Fs_batchsize))
    return res

# if __name__ == '__main__':
#     main()
def Fs_Densenet_train(Fs_epochs,Fs_lr,Fs_batchsize,Fs_layer,Fs_growth,train_path_idx,val_path_idx):
    Fs_accuracy,Fs_modelname=main(Fs_epochs,Fs_lr,Fs_batchsize,Fs_layer,Fs_growth,train_path_idx,val_path_idx)
    return Fs_accuracy,Fs_modelname
