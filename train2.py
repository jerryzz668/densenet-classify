...
best_prec1 = 0
train_dirs = '/home/adt/data/data/huawei_class/train_new/train9'
val_dirs = '/home/adt/data/data/huawei_class/train_new/val9'
def main():
...
def save_checkpoint(state, model, is_best, prec, epoch, filename='D_20200323_checkpoint.pth',
                    filename2='D_20200323_CP0.pth'):
    """Saves checkpoint to disk"""
    directory = "runs_resnet_18/%s/" % (args.name)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + "prec_" + str(prec) + "__epoch_" + str(epoch) + "__" + filename
    filename2 = directory + "prec_" + str(prec) + "__epoch_" + str(epoch) + "__" + filename2
    torch.save(state, filename)

    if is_best:
        torch.save(model, filename2)
        shutil.copyfile(filename,
                        'runs_resnet_18/%s/' % (args.name) + "prec_" + str(prec) + "__epoch_" + str(
                            epoch) + "__" + 'D_20200323_model_best.pth')
...
#     lr = args.lr * (0.1 ** (epoch // 150)) * (0.1 ** (epoch // 225)) * (0.1 ** (epoch // 350))
#     # log to TensorBoard
#     if args.tensorboard:
